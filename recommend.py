...
def train_cf_model(matrix):
    ...
    model = implicit.als.AlternatingLeastSquares(factors=N_FACTORS, iterations=N_ITERATIONS, calculate_training_loss=True, random_state=42 )
    model.fit(matrix)
...
def rerank(recommendations, restaurants, user_lat=None, user_lon=None, max_miles=None):
    ...
    if user_lat and user_lon:
        df["distance_miles"] = df.apply(lambda r: haversine(user_lat, user_lon, r["latitude"], r["longitude"]), axis=1)
        if max_miles:
    ...
    # use recency_weight instead of latest_review_days
    df["recency_score"] = df["recency_weight"].fillna(0)

    # has_response_rate not available, default to 0
    df["response_score"] = 0.0
...
def recommend(user_id, user_lat=None, user_lon=None, max_miles=None):
    ...
    reviews = pd.read_parquet(OUTPUT_DIR / "reviews.parquet")
    print("Build interaction matrix")
    matrix, _, item_map, user_to_idx, _ = build_interaction_matrix(reviews)
...
def run():
    ...
    print("Building interaction matrix from train...")
    ...
    print("Training CF model on train split...")
